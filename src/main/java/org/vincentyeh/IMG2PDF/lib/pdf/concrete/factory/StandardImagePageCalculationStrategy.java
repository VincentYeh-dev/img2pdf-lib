...
//			height fill
    ...
    private PointF calculatePosition(PageAlign align, SizeF object_size, SizeF container_size) {
        ...
        var x_space = container_size.width() - object_size.width();
        if (x_space < 0) throw new IllegalArgumentException("The space of x axis is negative.");

        var y_space = container_size.height() - object_size.height();
...
//        垂直 Y
        ...
        switch (hori_align) {
            ...
            case RIGHT -> position_x = x_space;
            case CENTER -> position_x = x_space / 2;
            default -> {
            }
        }

        switch (verti_align) {
            case BOTTOM -> position_y = 0;
            case TOP -> position_y = y_space;
            case CENTER -> position_y = y_space / 2;
            default -> {
            }
        }
